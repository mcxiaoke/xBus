...
我们的目标是从零开始自己写一个`EventBus`，参考上面的示例，我们首先定义接口，假设我们的接口叫`IBus`，实现类叫`Bus`，它最少需要三个方法。为了灵活和简洁，暂时不考虑性能，我们采用Annotation的方式指定事件接收者。
...
我们定义的`IBus`接口有三个方法`register(target)`，`unregister(target)`，`post(event)`，我们的事件接收器使用`Annotation`的方式指定，下面逐一考察这些问题：
